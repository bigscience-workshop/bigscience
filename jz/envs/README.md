...
find $six_ALL_CCFRWORK    -user `whoami` -type d ! \( -readable -executable \) -prune -o -type f -execdir chmod g+rw {} \; 2>&1 | egrep -v "(Operation not permitted)"
...
find $six_ALL_CCFRSCRATCH -user `whoami` -type d ! \( -readable -executable \) -prune -o -type f -execdir chmod g+rw {} \; 2>&1 | egrep -v "(Operation not permitted)"
...
find $six_ALL_CCFRSTORE   -user `whoami` -type d ! \( -readable -executable \) -prune -o -type f -execdir chmod g+rw {} \; 2>&1 | egrep -v "(Operation not permitted)"
```
